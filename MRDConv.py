...
class MRDConv(nn.Module):
    ...
    def _fuse_bn(self, branch):
        ...
        w = w * (gamma / var_sqrt).view(-1, 1, 1, 1) #融合公式
    ...
    def slim(self):
        ...
        oc_fusion, mid_c, _, _ = w_fusion.shape #mid_c==m-d_channels,这里=12
        ...
        print(b_sum.shape)
        # k_sum 尺寸: (C_mid, C_in, 5, 5)
        k_sum_flat = k_sum.view(mid_c, -1)   # 变成二维矩阵: (C_mid, C_in * 25)

        # w_fusion 尺寸: (C_out, C_mid, 1, 1)
        w_fusion_flat = w_fusion.view(self.out_channels, -1) # 变成二维矩阵: (C_out, C_mid)

        # 矩阵乘法核心！
        final_weight_flat = torch.matmul(w_fusion_flat, k_sum_flat) # 结果尺寸: (C_out, C_in * 25)
...
def show_map(M, title="", cmap="magma"):
    ...
    new_cmap = cmap(np.linspace(0.1, 0.6, 256))  # 截掉最亮部分
...
def visualize_mrdconv_weights(m: "MRDConv", mode="mean_abs"):
    ...
    # 1) 各分支 Conv+BN 融合后的原始核（注意：不是 5x5，是分支原核尺寸）
    ...
    # 2) 把每个分支嵌入到 5×5（和 slim() 一致），便于直观看“采样格点”
    mid_c = w_s3.shape[0]
    ic_in = m.in_channels
...
if __name__ == "__main__":
    ...
    visualize_mrdconv_weights(model, mode="mean_abs")  # or "l2"
    model.eval()  # 重要：BN使用running statistics
